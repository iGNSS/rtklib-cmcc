...
/* decode type 4049: proprietary message CMCC---------------------------------*/
static int decode_type4049(rtcm_t *rtcm)
{
	int i=24+12,ver=0,subtype=0;
	if (i+2+9>=rtcm->len*8) {
		trace(2,"rtcm3 4049: length error len=%d\n",rtcm->len);
        return -1;
	}
	ver    =getbitu(rtcm->buff,i,3); i+=3;
	...
	trace(2,"rtcm3 4049: unsupported message subtype=%d\n",subtype);
	return 0;
}
...
extern int decode_rtcm3(rtcm_t *rtcm)
{
	...
	switch (type) {
		...
		case 4049: ret=decode_type4049(rtcm); break;
