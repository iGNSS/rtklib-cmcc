...
//	RovAntPcv      ->Enabled=rel||ppp;
	RovAntPcv      ->Enabled=ppp;
	RovAnt         ->Enabled=(rel||ppp)&&RovAntPcv->Checked;
	RovAntE        ->Enabled=(rel||ppp);
	RovAntN        ->Enabled=(rel||ppp);
	RovAntU        ->Enabled=(rel||ppp);
	LabelRovAntD   ->Enabled=(rel||ppp);
...
//	LabelRovAntD   ->Enabled=(rel||ppp)&&RovAntPcv->Checked&&RovAnt->Text!="*";
	...
	RovPosTypeP    ->Enabled=PosMode->ItemIndex==PMODE_FIXED||PosMode->ItemIndex==PMODE_PPP_FIXED||PosMode->ItemIndex==PMODE_KINEMA||PosMode->ItemIndex==PMODE_STATIC;
